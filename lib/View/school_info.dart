...
class _SchoolInfoState extends State<SchoolInfo> {
  ...
  Widget build(BuildContext context) {
    var provider=Provider.of<SchoolProvider>(context);
    var data=provider.schoolDataModel;
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: 120,
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            reusableTextField(data?.schoolName??"", Icons.school),
            const SizedBox(height: 15,),
            reusableTextField(data?.schoolAddress??"", Icons.location_on_rounded),
            const SizedBox(height: 15,),
            reusableTextField(data?.numberOfStudents??"", Icons.person),
            const SizedBox(height: 15,),
            reusableTextField(data?.numberOfTeachers??"", Icons.person),
            Padding(
